...
module.exports = (function(){
  ...
  DeferredResponse.prototype.subscribe = function(target,eventName,options){
    var _this = this;
    this._subscribed = true;
    var localName = (options.forward_as) ? options.forward_as : eventName;
  ...
  DeferredResponse.prototype.resolve = function(val){
    if (!this._finished) {
      this._finished = true;
      this.res.out({route:"/",data:val,req_id:this.req_id,subscribed:this._subscribed},{});
    } else {
  ...
  DeferredResponse.prototype.reject = function(err){
    if (!this._finished) {
      this._finished = true;
      this.res.out({route:"/",error:err,req_id:this.req_id,subscribed:this._subscribed},{});
    } else {
  ...
  DeferredResponse.prototype.end = function(){
    if (!this._finished) {
      this._finished = true;
      this.res.out({route:"/",req_id:this.req_id},{});
    } else {
