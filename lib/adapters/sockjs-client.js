...
function SockjsClient(options) {
  ...
  this.sock.onopen = function(){
    console.info('midsockets.SockjsClient :: opened '+options.url);
  ...
  this.sock.onmessage = function(e){ 
    if (!e) { throw new Error("midsockets.SockjsClient :: Strange Message of 'undefined'"); }
    if (e.type !== "message") { throw new Error("midsockets.SockjsClient :: Strange Message of type: '"+e.type+"'"); }
    try {
      var parsed = JSON.parse(e.data);
    } catch(err) {
      throw new Error("midsockets.SockjsClient :: Could not parse message data: '"+e.data+"'");
    }
    _this._in(parsed, {});
  };
